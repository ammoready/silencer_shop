...
describe SilencerShop::Catalog do
  ...
  before do
    ...
    stub_request(:get, "https://silencershopportaldebug.azurewebsites.net/api/productfeed/catalog").
      with(headers: get_headers).to_return(status: 200, body: FixtureHelper.get_fixture_file('silencer-shop-catalog.json').read)
  end

  describe '.all' do
    it 'returns an array of all items' do
      items = SilencerShop::Catalog.all(options)

      items.each_with_index do |item, index|
        case index
        when 0
          expect(item[:upc]).to  eq('817272010015')
          expect(item[:name]).to eq('PART, DELTA O-RING PACK')
        when 1
          expect(item[:upc]).to  eq('817272010138')
          expect(item[:name]).to eq('PART, DELTA THREAD PROTECTOR 1/2-28')
        end
      end
